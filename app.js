...
document.getElementById('btnGoogleLogin').addEventListener('click', async () => {
  ...
  try {
    ...
    // Si el popup falla por COOP/bloqueado, usar redirect
    if (e.code === 'auth/popup-blocked' || e.code === 'auth/popup-closed-by-user' || e.message?.includes('Cross-Origin')) {
      try {
        await signInWithRedirect(auth, new GoogleAuthProvider());
        return; // la página se recargará sola
      } catch(e2) {
        btn.disabled = false;
        btn.innerHTML = googleSVG;
        showLogin('Error al iniciar sesión. Intenta de nuevo.');
        return;
      }
    }
    btn.disabled = false;
...
// Manejar resultado de redirect (cuando la página recarga después de signInWithRedirect)
