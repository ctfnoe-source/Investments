...
function renderMovimientos(){
  ...
  document.getElementById('page-movimientos').innerHTML=`
    ...
    <div class="filter-pills">
      ...
      <input class="pill-search" placeholder="Buscar..." value="${movFilter.search}" oninput="movFilter.search=this.value;debouncedRenderMovimientos()">
...
function editPlatField(id,field,el,inputType){
  ...
  input.onkeydown=e=>{ if(e.key==='Enter'){ e.preventDefault(); input.blur(); } if(e.key==='Escape'){ _cancelled=true; saveAll(); } };
  if(inputType==='moneda')input.onchange=finish;
  el.replaceWith(input);input.focus();
}
function deletePlatform(id){
  const p=platforms.find(x=>x.id===id); if(!p) return;
  showConfirm('¿Eliminar <strong>'+escHtml(p.name)+'</strong>?','Se borrará también su historial de movimientos. Esta acción no se puede deshacer.',()=>{ platforms=platforms.filter(x=>x.id!==id); movements=movements.filter(m=>!(m.seccion==='plataformas'&&m.platform===p.name)); saveAll(); showToast('🗑 '+p.name+' eliminada','info'); });
}
...
function deleteGoal(id){
  ...
  showConfirm('¿Eliminar la meta <strong>'+escHtml(g.nombre)+'</strong>?','Esta acción no se puede deshacer.',()=>{ goals=goals.filter(x=>x.id!==id); saveAll(); showToast('🎯 Meta eliminada','info'); });
}
